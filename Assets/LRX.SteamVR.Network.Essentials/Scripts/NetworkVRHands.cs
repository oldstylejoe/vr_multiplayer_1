...
// It has been modified so that the right hand always has a gun.
...
public class NetworkVRHands : NetworkBehaviour
{
	...
	{
        // Modifications for the right hand being the main gun made here. 
        // Essentially, the hand inherits the Usable property and has the GunItem script attached,
        // so the hand's fire method is called when the trigger is pressed. 

        // Bug: There is a possibility the hands will sync such that the left hand gains the functionality of
        // the right hand, but the right hand still fires the bullet. So, the left trigger affects the right hand's gun.
		Debug.Log ("Trigger pressed");
        if (this.tag == "Right Hand")
        {
            CmdStartUsing(this.netId, this.netId);
        }
        else
        {
            // If this is not the right hand's trigger, then the trigger works how it was implemented.
